			...
			steps {
				script {
					...
					if("${APP_ENV}" == "dev"){
						...
						sh "scp dist/dvdtheque-frontend/* jenkins@$DEV_SERVER_IP:/var/www/dvdtheque-frontend"
					}else if ("${APP_ENV}" == "production") {
						sh "ssh jenkins@$PROD_SERVER_IP rm -rf /var/www/dvdtheque-frontend/*"
						sh "scp dist/dvdtheque-frontend/* jenkins@$PROD_SERVER_IP:/var/www/dvdtheque-frontend"
					}
				}
			}
		}
